    ...
    function ResourceEvents(props) {
        ...
        _this.initDrag = function (ev) {
            ...
            if (viewType !== 1) {
                ...
                var leftIndex = Math.floor(startX / cellWidth); //topIndex =  Math.floor(startY / 8.8)
                var left = leftIndex * cellWidth;
                ...
                var currentX = ev.clientX - pos.x; //ev.clientY - pos.y
                 ...
                 var topIndex =  Math.floor(startY / 9)
                 var top = (topIndex * 9)
                 var bottomIndex = Math.ceil(startY / 9);
                 var height = (bottomIndex - topIndex) * 9
        ...
        _this.doDrag = function (ev) {
            ...
            if (viewType !== 1) {
                ...
                var currentY = ev.clientY - pos.y; //ev.clientY - pos.y
                var topIndex = Math.floor(Math.min(startY, currentY) / 9); // startX, CurrentX /8
                topIndex = topIndex < 0 ? 0 : topIndex; 
                var top = (topIndex * 9); //8.8
                var bottomIndex = Math.ceil(Math.max(startY, currentY) / 9);
                bottomIndex = bottomIndex > 24 ? 24 : bottomIndex;
                var height = (bottomIndex - topIndex) * 9;
                _this.setState({
                    leftIndex: leftIndex,
                    topIndex: topIndex,
        ...
        _this.stopDrag = function (ev) {
            ...
            var startTime = viewType !== 1 ? headers[leftIndex].time : localeMoment(headers[leftIndex].time).add(topIndex, 'hours').format(_index.DATETIME_FORMAT);//.add(topIndex, hours)
            var endTime = viewType !== 1 ? resourceEvents.headerItems[rightIndex - 1].end : localeMoment(headers[leftIndex].time).add(bottomIndex, 'hours').format(_index.DATETIME_FORMAT);//headers[leftIndex].time.add(bottomIndex, hours)
                ...
                if (headerItem.count > 0 || headerItem.summary != undefined) {
                    ...
                    headerItem.events.forEach(function (evt, idx) {
                        if (idx < renderEventsMaxIndex && evt !== undefined && evt.render) {
                            ...
                            var top2 = eventStart.format('H') * 9
